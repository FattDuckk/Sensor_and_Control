...
Contour_Path = 'Students\Data2_Soft_Pullback_1\Contours\';
EM_Path = 'Students\Data2_Soft_Pullback_1\EM\';
...
%Turn this on/off to sketch the reconstructed aorta contour
plot_ring = false;
...
disp(distance_array);
    ...
    ylabel('Y-axis');
    zlabel('Z-axis');
%     title('3D Path of Transformed Points');
%     % Plot distance between centerline and path in 2D
% figure;
% plot(distance_array);
% xlabel('Point Number'); 
% ylabel('Distance (mm)');
% title('Distance Between Centerline and Path');
% Plot the distance as a line connecting the catheter and robot path
for i = 1:file_length
...
% % Display the distance values as text annotations
% for i = 1:file_length
%     text(cenX(i), cenY(i), cenZ(i), num2str(distance_array(i)), 'FontSize', 8, 'Color', 'g');
% end